"""右括号 ) (目标右侧,弧凸向远离字、开口朝字),复用 circle_path 半弧。"""
...
def rparen(target, size=None, color=None, thinning=None, cap=None, taper=None,
    ...
    def build():
        ...
        cx = target.get_right()[0] + gap
        r = _height(target) * ratio
        # 半弧凸向右(远离字、开口朝字):start π/2, direction -1, sweep π, 经 0(右最远点) = ) 从上画到下
    ...
    return _make_mark(target, "rparen", build, speed=speed, seed=seed, variation=variation)